...
public class TaskCreatedEventHandler(ISchedulerFactory schedulerFactory) : INotificationHandler<TaskCreatedDomainEvent>
{
    public async Task Handle(TaskCreatedDomainEvent notification, CancellationToken cancellationToken)
    {
        ...
        {
            await JobFactory.CreateJob(task, tableName, scheduler, minutes: 5);
            await JobFactory.CreateJob(task, tableName, scheduler, hours: 5);
            await JobFactory.CreateJob(task, tableName, scheduler, days: 1);
        ...
        await JobFactory.CreateJob(task, tableName, scheduler);
